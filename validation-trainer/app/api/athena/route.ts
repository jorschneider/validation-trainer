...
Remember: Your responses should feel like a real person reacting to how well their partner understands them.`;
    ...
    const response = completion.choices[0]?.message?.content || "I don't know what to say.";

    return NextResponse.json({ 
      response: response.trim(),
      validationQuality: qualityAnalysis?.validationQuality || 'poor' // Return for potential UI feedback
    });
  } catch (error) {
